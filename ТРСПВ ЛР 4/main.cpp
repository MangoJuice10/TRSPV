...
int main(int argc, char* argv[]) {
	...
	if (rank == SR_BASE_RANK) server();

	if (rank >= CL_BASE_RANK && rank < CL_BASE_RANK + CL_NUM) client(rank);

	if (rank >= RS_BASE_RANK && rank < RS_BASE_RANK + RS_NUM) resource(rank);
