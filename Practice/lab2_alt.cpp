...
void lab2_alt(int argc, char* argv[]) {
	MPI_Init(&argc, &argv);
	int rank;
	...
	if (rank == 0) {
		...
		double x = 0;
		for (size_t i = 0; i < TOTAL_BLOCKS_NUM; i++, x += 0.1) {
			double y = 0;
			for (size_t j = 0; j < ELS_PER_BLOCK; j++, y += 0.1) {
				blocks[i][j] = sin(x) + exp(y);
			}
		}

		for (size_t i = 0; i < TOTAL_BLOCKS_NUM; i++) {
			int blockIdx = i + 1;
			for (int j = 0; j < ELS_PER_BLOCK; j++) {
				if (blockIdx % 2 == 0) evenBlocks[(blockIdx - 1) / 2][j] = blocks[i][j];
				else oddBlocks[(blockIdx - 1) / 2][j] = blocks[i][j];
			}
		}
